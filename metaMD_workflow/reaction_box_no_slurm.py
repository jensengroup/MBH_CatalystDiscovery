...
def write_md_input(scale_factor, time=2, constrain_bond=None):
    """
    ...
    if constrain_bond:
        ...
        $constrain
           distance: {0},{1},auto
        $end
        """.format(constrain_bond[0], constrain_bond[1])
        with open("md.inp", 'r') as f:
            contents = f.readlines()
        contents.insert(9, _string)
        with open("md.inp", 'w') as f:
            contents = "".join(contents)
            f.write(contents)


def write_meta_input(time, kpush, alp, scale_factor, constrain_bond=None, structure_file=None):
...
def calculate_md_relaxed_structure(smiles, scale_factor, ridx,
                                   constrain_bond=None, solvent="--gbsa methanol"):
    """
    ...
    write_md_input(scale_factor, constrain_bond=constrain_bond)
    output = run_cmd("/groups/kemi/koerstz/opt/xtb/6.1/bin/xtb {0} --omd --input md.inp --gfn2 {2} --chrg {1}".format(str(ridx)+'.xyz', charge, solvent))
    ...
    with open(out_file, 'r') as _file:
        line = _file.readline()
        line = _file.readline()
        last_energy = np.float(line.split()[0])
    check_md_reaction(out_file, charge, smiles, str(ridx)+'.xyz')
    shutil.copy(out_file, '../')

    os.chdir('../')
    return out_file, charge, n_atoms, last_energy
...
def extract_smiles(xyz_file, charge, allow_charge=True, check_ac=False,
    ...
    if check_can_smiles:
        global CAN_SMILES_SAME
        can_smiles = canonicalize_smiles(structure_smiles)
        if CAN_SMILES != can_smiles:
            CAN_SMILES_SAME = False
            print("change in canonical smiles")
...
def get_smiles(xyz_file, charge, check_ac=False, check_can_smiles=False):
    """
    ...
    try:
        smiles, formal_charge, res_status = extract_smiles(xyz_file, charge,
                                                           allow_charge=True,
                                                           check_ac=check_ac,
                                                           check_can_smiles=check_can_smiles)
        if formal_charge != charge:
            print("trying as radical")
            smiles, formal_charge, res_status = extract_smiles(xyz_file, charge,
                                                               allow_charge=False,
                                                               check_ac=check_ac,
                                                               check_can_smiles=check_can_smiles)
...
def check_md_reaction(md_xyz, charge, in_smiles, in_xyz):
    ...
    if smiles != in_smiles or smiles != optsmiles:
        ...
        if AC_SAME == False and CAN_SMILES_SAME == False:
            global DF
        ...
        save_structure_to_database(in_smiles, "xtbopt.xyz", res_status2,
                                   database_dir)
...
def split_trajectory(last_energy, trajectory_file, charge, current_time, n_check=10):
    """
    ...
    with open(trajectory_file, 'r') as _file:
        line  =_file.readline()
        line = _file.readline()
        first_energy = np.float(line.split()[0])
    if first_energy - last_energy > 0.1:
        return n_steps_list
...
def update_metadyn_input(file_name, old_s, new_s):
    """
    changes the scaling factor in the input file
    """
    ...
    for i, line in enumerate(contents):
        if "autoscale" in line:
            new_line = line.replace(str(old_s), str(new_s))
            line_idx = i
...
#make function for pushing fragments closer - if error from reading MOL -> do
#not move fragments
def move_fragments_closer(current_time, mol):
    """
    Push fragments 0.1Å closer together
    """
    atoms, _, xyz_coordinates = xyz2mol_local.read_xyz_file("restart_structure_{0}.xyz".format(current_time))
    charge = Chem.GetFormalCharge(mol)
    try:
        input_mol = xyz2mol_local.xyz2mol(atoms, xyz_coordinates, charge=charge,
                                          use_graph=True,
    ...
    dmatrix = rdmolops.Get3DDistanceMatrix(input_mol)
    atoms_frag1, atoms_frag2 = Chem.GetMolFrags(mol)
    print(atoms_frag1, atoms_frag2)
    min_dist = None
    c = list(itertools.product(atoms_frag1, atoms_frag2))
    for a1, a2 in c:
        _dist = dmatrix[a1, a2]
        if not min_dist or _dist < min_dist:
            min_dist = _dist
            dist_pair = (a1, a2)
    print("minimum distance = ", min_dist, dist_pair)
    if min_dist < 1.9:
        move_dist = 0.0
    else:
        move_dist = 0.1
    print("changing atom positions")
    conf = input_mol.GetConformer()
    xyz_1 = conf.GetAtomPosition(dist_pair[0])
    xyz_2 = conf.GetAtomPosition(dist_pair[1])
    trans_vector = xyz_1 - xyz_2
    trans_vector = 1/min_dist * trans_vector
    for atom_idx in atoms_frag2:
        coord = conf.GetAtomPosition(atom_idx)
        coord += Point3D(*(trans_vector*(move_dist)))
        conf.SetAtomPosition(atom_idx, coord)

    rdmolfiles.MolToXYZFile(input_mol, "restart_structure_adjusted_{0}.xyz".format(current_time))


def setup_restart(mol, time_ps, n_atoms, current_time, structure_file, s=0.8,
                  with_products=False, multi_fragment=False):
    """
    goes through the structures saved every ps and appends to
    coordinate file
    ...
    extract_last_structure("xtb.trj", "restart_structure_{0}.xyz".format(current_time))

    #move fragments closer if they moved too far away in the simulation
    try:
        move_fragments_closer(current_time, mol)
    except:
        shutil.copy("restart_structure_{0}.xyz".format(current_time),
                    "restart_structure_adjusted_{0}.xyz".format(current_time))


    shutil.copy("xtb.trj", "xtb_"+str(current_time)+".trj")


    update_metadyn_input("metadyn.inp", s, new_s)
    ...
    with open("restart_structure_{0}.xyz".format(current_time), 'r') as _file:
        line = _file.readline()
        line = _file.readline()
        last_energy = np.float(line.split()[0])


    return new_s, last_energy


def do_metadynamics_calculation(last_energy, md_file, time, k_push, alp, scale_factor,
                                charge, current_time, constrain_bond=None,
                                structure_file=None, solvent="--gbsa methanol"):
    """
    ...
    if not RESTARTED:
        ...
        write_meta_input(time, k_push, alp, scale_factor,
                         constrain_bond=constrain_bond,
                         structure_file=structure_file)

    output = run_cmd("/groups/kemi/koerstz/opt/xtb/6.1/bin/xtb {0} --md --input metadyn.inp --gfn2 {2} --chrg {1}".format(md_file, charge, solvent))

    n_steps_list = split_trajectory(last_energy, 'xtb.trj', charge, current_time)
    while not n_steps_list:
        new_scale_factor = scale_factor + 0.02
        update_metadyn_input("metadyn.inp", scale_factor, new_scale_factor)
        output = run_cmd("/groups/kemi/koerstz/opt/xtb/6.1/bin/xtb {0} --md --input metadyn.inp --gfn2 {2} --chrg {1}".format(md_file, charge, solvent))
        n_steps_list = split_trajectory(last_energy, 'xtb.trj', charge,
                                        current_time)
        scale_factor = new_scale_factor
        if scale_factor > 1.1:
            sys.exit("Error: something wring with starting structure - steep energy increase in beginning of MD")
    ...
    for xyz_file in xyz_files:
        output = run_cmd("/groups/kemi/koerstz/opt/xtb/6.1/bin/xtb {0} --opt tight --gfn2 {2} --chrg {1}".format(xyz_file, charge, solvent))
...
if __name__ == "__main__":
    ...
    SCALE_FACTOR = np.float(sys.argv[4])
    ...
    DF = pd.DataFrame()
    AC_SAME = True
    CAN_SMILES_SAME = True
    CURRENT_TIME = 0

    MULTI_FRAGMENT = not len(SMILES.split('.')) == 1
    #MULTI_FRAGMENT=False

    MD_FILE, CHARGE, N_ATOMS, LAST_ENERGY = calculate_md_relaxed_structure(SMILES, SCALE_FACTOR,
                                                                            SMILES_IDX,
                                                                            constrain_bond=CONSTRAIN_BOND)

    while (CAN_SMILES_SAME or AC_SAME) and (CURRENT_TIME < MAX_TIME):
        SCALE_FACTOR = do_metadynamics_calculation(LAST_ENERGY, MD_FILE, TIME, K_PUSH, ALP,
                                                   SCALE_FACTOR, CHARGE, CURRENT_TIME,
                                                   constrain_bond=CONSTRAIN_BOND,
                                                   structure_file=STRUCTURE_FILE)

        STRUCTURE_FILE = str(SMILES_IDX)+'_initial_structures.xyz'
        SCALE_FACTOR, LAST_ENERGY = setup_restart(MOL, TIME, N_ATOMS, CURRENT_TIME,
                                                   STRUCTURE_FILE, s=np.float(SCALE_FACTOR),
                                                   with_products=WITH_PRODUCTS,
                                                   multi_fragment=MULTI_FRAGMENT)
        MD_FILE = "restart_structure_adjusted_{0}.xyz".format(CURRENT_TIME)
