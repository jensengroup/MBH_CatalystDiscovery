...
def get_single_step_df_can(can_smiles, df):
    """
    ...
    df_reverse = df[df.Products_canonical == can_smiles]
    df_forward = df[df.Reactants_canonical == can_smiles]
    ...
    i = len(df_forward)
    for idx, product, reactant in zip(df_reverse.index, df_reverse.reactant_smiles_am, df_reverse.product_smiles_am):
        if product not in df_forward.product_smiles_am.to_list():
            df_forward.loc[i, 'reactant_smiles_am'] = reactant
            df_forward.loc[i, 'product_smiles_am'] = product
            df_forward.loc[i, 'Reactions'] = reactant+'>>'+product
...
def combined_run_dfs(smiles_idx, smiles, can_smiles, n_runs):
    """
    ...
    if len(combined_df) != 0:
        ...
        df_forward_1step_can = get_single_step_df_can(can_smiles,
                                                      combined_df_final)
    ...
    df_forward_1step_can.to_csv(str(smiles_idx)+'_1step_can.csv')
    #print(df_forward_1step)
    #print(df_forward_1step_can)
...
if __name__ == '__main__':
    ...
    for IDX, SMILES in zip(IN_SMILES_DF.index, IN_SMILES_DF.smiles):
        ...
        output = combined_run_dfs(IDX, SMILES, CAN_SMILES, N_RUNS)
